...
protocol UserPresenterProtocol : PresenterProtocol {
     ...
     func addRecordToDatabase( _ record:UserRecord?)->Bool
}
...
extension UserPresenter {
    func lookupClosestMatchingRecord( _ record:UserRecord?)->Bool {
        ...
        guard let imageData = record?.photo , let name = record?.tag else {
            print("Invalid input parameters")
            return false
        }
        
        DispatchQueue.global().async {
            let results = self.dbMgr.useRegisteredModelToFindMatchingItemsInDBToInputImage(imageData, imageName: name, propertyName: UserRecordDocumentKeys.photo.rawValue)
    ...
    func addRecordToDatabase( _ record:UserRecord?)->Bool {
        ...
        DispatchQueue.global().async {
            let result = self.dbMgr.addItemToDBWithPredictionResult(imageData, imageName: name, propertyName: UserRecordDocumentKeys.photo.rawValue)
            
            DispatchQueue.main.async {
                if result == true {
                    self.associatedView?.showAlertWithTitle(nil, message: "Succesfully added image")
                }
                else {
                    self.associatedView?.showAlertWithTitle(nil, message: "Failed to add image")
                }
            }
