...
class ItemTableViewController:UITableViewController, UserPresentingViewProtocol {
    ...
    override public func viewDidLoad() {
        ...
        self.title = NSLocalizedString("User Lookup", comment: "")
...
extension ItemTableViewController {
    @IBAction func onDoneTapped(_ sender: UIBarButtonItem) {
       userRecord = UserRecord.init()
        ...
        userRecord?.name = self.nameTextEntry?.text
//        if let imageVal = self.userImageView?.image, let imageData = imageVal.jpegData(compressionQuality: 1.0)  {
    ...
    override public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case Section.basic.index :
            ...
            case BasicRows.name.index :
                ...
                if let name = self.userRecord?.name as? String {
...
extension ItemTableViewController {
    func updateUIWithMatchingRecords(_ records: [UserRecord]?, error: Error?) {
        switch error {
            case nil:
            // TODO:  We may need to navigate to a new table view to show the matches
