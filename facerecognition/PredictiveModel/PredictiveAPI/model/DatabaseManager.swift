...
class DatabaseManager {
    ...
    // predictions
    fileprivate var openfacePredictiveModel = OpenFacePredictiveModel.init()
    ...
    deinit {
        ...
        do {
            ...
            openfacePredictiveModel.unregisterModel()
            try self._db?.close()
        }
        catch  {
...
extension DatabaseManager {
    ...
    func registerPredictionModel() {
        print(#function)
        openfacePredictiveModel.registerModel()
    }
    ...
    // For searches on type property
        ...
        let model = OpenFacePredictiveModel.name
...
extension DatabaseManager {
    ...
    func useRegisteredModelToFindClosestMatchesInDBToInputImage(_ imageData:Data, imageName:String, propertyName:String)->(userRecords:[UserRecord]?,status:Bool) {
        ...
        let modelName = OpenFacePredictiveModel.name
        
        // Fingerprint of input image
        let inputPhotoParam = Expression.dictionary([propertyName : Expression.parameter(imageName)])
        let fingerPrint1 = Function.prediction(model: modelName, input: inputPhotoParam).property(propertyName)
                
        // Fingerprints of images in database
        let photoInDB = Expression.dictionary([propertyName : Expression.property(propertyName)]);
        let fingerPrint2 = Function.prediction(model: modelName, input: photoInDB).property(propertyName)
    ...
    func useRegisteredModelToFindFingerprintInDBForInputImage(_ imageData:Data, imageName:String, propertyName:String)->(userRecords:[UserRecord]?,status:Bool) {
        print(#function)
        guard let db = db else {
            fatalError("Database not initialized")
            return (nil,false)
        ...
        // COnstruct the query and return the top 5 matching results
        let query = QueryBuilder
            .select(SelectResult.expression(fingerPrint1))
            .from(DataSource.database(db))
        ...
        do {
            ...
            print("Time to lookup matching image in database is \(timeInterval) seconds")
            print ("results are ...")
