...
func loadImageFilesFromFolder(_ folder:String)->[String:UIImage] {
    ...
    while let element = enumerator?.nextObject() as? String {
        if(enumerator?.fileAttributes?[FileAttributeKey.type] as! FileAttributeType == FileAttributeType.typeRegular){
            ...
            if let data = UIImage(contentsOfFile: newPath.appending("/\(element)")) {
                ...
                if let last = element.components(separatedBy: "/").last {
                    files[last] = data
                }
    ...
    func loadImageFilesFromFolder_Orig(_ folder:String)->[String:UIImage] {
        var files:[String:UIImage] = [String:UIImage]()
        let bundle = Bundle(for: type(of: self))
        let path = bundle.bundlePath
        let newPath = path.appending("/\(folder)")
        ...
        while let element = enumerator?.nextObject() as? String {
            if(enumerator?.fileAttributes?[FileAttributeKey.type] as! FileAttributeType == FileAttributeType.typeRegular){
                ...
                if let data = UIImage(contentsOfFile: newPath.appending("/\(element)")) {
                    //openface needs this size
                    print("filename is \(element)")
                    if (element.components(separatedBy: "/").last?.contains("0001")) != nil {
                        let last = element.components(separatedBy: "/").last
                        if let indexOf0001 = last?.components(separatedBy: "_").lastIndex(of: "0001.jpg"),let name = element.components(separatedBy: "/").first  {
                            let resizedData = resize(image: data, newWidth: 96.0, newHeight: 96.0)
                            print("last is \(last).Index of 0001 is  \(last?.components(separatedBy: "_").lastIndex(of: "0001.jpg"))")
                            files[name] = data
                        }
