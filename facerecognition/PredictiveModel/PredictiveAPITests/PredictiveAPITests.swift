    ...
    func testPerformanceOpenFacePrediction() {
        ...
        self.measure {
            for (name,image) in images {
                let _ = try? faceRecognitionService.runOpenFaceModelOnImage(image)
        
                }
            }
    }
    
    func testPerformanceOpenFacePredictionBatched() {
        
        // Includes cost
        // converting images to OpenFaceInput
        // Running predictins
        // converting OpenFaceOutput's MLMultiArray results to doublearray
        // measure only cost of prediction
        self.measure {
            // 2. Run batched prediction on all images
             _ = faceRecognitionService.runOpenFaceModelOnImages([UIImage](images.values))
        }
      }
}
